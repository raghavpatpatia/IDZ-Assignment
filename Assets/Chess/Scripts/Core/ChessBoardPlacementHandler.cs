...
public sealed class ChessBoardPlacementHandler : GenericMonoSingleton<ChessBoardPlacementHandler> {
    ...
    internal void Highlight(int row, int col) {
        ...
        Instantiate(tileSO.HighlightPrefab, tile.transform.position, Quaternion.identity, tile.transform);
    }

    internal void CaptureHighlight(int row, int col)
    {
        var tile = GetTile(row, col).transform;
        if (tile == null)
        {
        ...
        Instantiate(tileSO.CaptureHighlightPrefab, tile.transform.position, Quaternion.identity, tile.transform);
    }
